    ...
    <tbody>
        @foreach($productDetail as $product)
                <tr>
                    ...
                    <td id="td-color">{{ $product->color->name }}<input type="hidden" value="{{ $product->color->id }}"
                            id="color-id" /></td>
                    <td id="td-capacity">{{ $product->capacity->name }}<input type="hidden" value="{{ $product->capacity->id }}"
                            id="capacity-id" /></td>
                    ...
                    @if($activeDiscount)
                        ...
                        <td>{{ $activeDiscount->price_formatted }}<input type="hidden" value="{{ $activeDiscount->price }}"
                                name="price" id="price-id" /></td>
                    @else
                        <td>0</td>
                        <td>{{$product->price_formatted}}<input type="hidden" value="{{ $product->price }}" name="price"
                                id="price-id" /></td>
                    @endif

                    <td><input type="number" max="{{ $product->quantity }}" value="1" min="1" name="quantity"
                            id='quantity-id' /></td>
