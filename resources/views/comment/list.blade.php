                ...
                @foreach($listComment as $Comment)
                        <tr>
                            <td>
                                ...
                                @endif
                                <div class="product-name">{{ $Comment->product->name }} - {{ $Comment->color->name }} -
                                    {{ $Comment->capacity->name }}</div>
                            </td>
                            <td class="comment-container">
                                <div class="comment-section">
                                    <strong class="comment-author">{{ $Comment->customer->name }}
                                        <span
                                            class="comment-date">{{ \Carbon\Carbon::parse($Comment->date)->format('d/m/Y H:i') }}</span>
                                    </strong>
                                    <div class="comment-content">{{ $Comment->content }}</div>
                                </div>
                                @foreach($Comment->comment_detail as $detail)
                                    <div class="comment-detail-section">
                                        <strong class="comment-author">{{ $detail->admin->name }}
                                            <span
                                                class="comment-date">{{ \Carbon\Carbon::parse($detail->date)->format('d/m/Y H:i') }}</span>
                                        </strong>
                            ...
                            <td>
                                ...
                                @else
                                    <button class="btn btn-primary showModalButton" data-id="{{ $Comment->id }}"><i
                                            class="fa-solid fa-reply"></i> Trả lời</button>
                                @endif
                        ...
                        <form method="POST" action="{{ route('comment.hd-rep') }}" id="replyForm">
                            ...
                            <div class="md-footer">
                                <button type="button" class="btn btn-secondary"
                                    id="closeModalButton">Thoát</button>&nbsp;&nbsp;
